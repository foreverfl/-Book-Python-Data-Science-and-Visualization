...
# 1. Inner Join: Dataframe 간 조인 조건을 만족하는 행을 합치는 것.
# (1) One-to-One
...
print("Inner Join (One-to-Many) 결과:")
print(inner_join_many_df)

# 2. Outer Join: 조건에 부합하지 않는 행까지 포함시켜 결합하는 방법. NaN 처리를 함.
# 데이터프레임 df1과 df2의 'ID'를 기준으로 Outer Join
outer_join_df = pd.merge(df1, df2, on='ID', how='outer')
print("Outer Join 결과:")
print(outer_join_df)

# 3. Left Join: 첫 번째 Dataframe을 기준으로 두 번째 Dataframe을 결합하는 방법.
# 데이터프레임 df1을 기준으로 Left Join
left_join_df = pd.merge(df1, df2, on='ID', how='left')
print("Left Join 결과:")
print(left_join_df)

# 4. Right Join: 첫 번째 Dataframe을 기준으로 첫 번째 Dataframe을 결합하는 방법.
