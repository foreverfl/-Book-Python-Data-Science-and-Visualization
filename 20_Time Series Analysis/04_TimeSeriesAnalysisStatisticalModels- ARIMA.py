...
# 임의의 시계열 데이터 생성
np.random.seed(42)
n_points = 100  # 데이터 포인트 개수를 100개로 설정
# 0에서 10 사이의 값을 균일한 간격으로 100개 생성. 시계열 데이터의 독립 변수
...
# 자기회귀 모델 학습 (여기서는 2차 자기회귀를 사용)
lag = 2  # 시계열 데이터의 현재 값이 이전 2개의 데이터 포인트에 의존
...
plt.plot(np.arange(start, end + 1), forecast, label='Forecast', color='red')
plt.legend()
plt.title('AutoRegressive Model 예제')
plt.show()
